 ...
 *  图片编辑滤镜控制器委托
 */
@protocol TuSDKPFEditFilterControllerDelegate <TuComponentErrorDelegate>
/**
 *  图片编辑完成
 *
 ...
 *  @param result 处理结果
 */
- (void)onTuSDKPFEditFilter:(TuSDKPFEditFilterController *)controller result:(TuResult *)result;
@optional
/**
 *  图片编辑完成  (异步方法)
 *
 ...
 */
- (BOOL)onAsyncTuSDKPFEditFilter:(TuSDKPFEditFilterController *)controller result:(TuResult *)result;
@end
 ...
 *  图片编辑滤镜控制器
 */
@interface TuSDKPFEditFilterController : TuEditFilterControllerBase<TuSDKPFEditFilterGroupDelegate>{
@protected
    // 默认样式视图
