 ...
 */
typedef void (^TuEditComponentBlock)(TuResult *result, NSError *error, UIViewController *controller);

#pragma mark - TuEditComponent
/**
 *  组件基础类
 */
@interface TuEditComponent : NSObject<TuComponentErrorDelegate>
 ...
 *  组件回调
 */
@property (nonatomic, strong) TuEditComponentBlock callbackBlock;
 ...
 */
+ (instancetype)initWithSourceController:(UIViewController *)controller callbackBlock:(TuEditComponentBlock)callbackBlock;
 ...
 *  @param controller  最后执行的控制器
 */
- (void)notifyResult:(TuResult *)result
               error:(NSError *)error
          controller:(UIViewController *)controller;
@end

#pragma mark - TuInputComponent
/**
 *  组件基础类(输入内容)
 */
@interface TuInputComponent : TuEditComponent
/**
