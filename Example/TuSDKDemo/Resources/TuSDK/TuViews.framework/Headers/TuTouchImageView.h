...
@class TuTouchImageView;
@protocol TuTouchImageViewInterface;

/**
 *  图片编辑视图 (旋转，缩放)委托
 */
@protocol TuTouchImageViewDelegate <NSObject>
/**
 *  动画状态改变
 *
 ...
 */
- (void)onTuSDKICTouchImageView:(UIView<TuTouchImageViewInterface> *)view inAniming:(BOOL)inAniming;
@end

/**
 *  图片编辑视图接口
 */
@protocol TuTouchImageViewInterface <NSObject>
/**
 *  图片编辑视图 (旋转，缩放)委托
 */
@property (nonatomic, weak) id<TuTouchImageViewDelegate> delegate;
 ...
 *  图片编辑视图 (旋转，缩放)
 */
@interface TuTouchImageView : UIView<UIScrollViewDelegate, TuTouchImageViewInterface>{
@protected
    // 旋转和裁剪 裁剪区域视图
