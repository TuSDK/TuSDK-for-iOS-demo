...
#pragma mark - TuTSAssetInterface
 ...
 *
 */
- (void)asyncFullResolutionImage:(TuTSAssetProgressBlock)block;
...
#pragma mark - TuTSAssetsGroupInterface
/**
 *  排序字段
 */
 ...
 *  @return 该组所有相片信息
 */
-(NSArray<TuTSAssetInterface> *) allPhotosWithSortKeyType:(lsqAssetSortKeyType) sortKeyType ascending:(BOOL) ascending;
 ...
 *  @param error  是否返回错误信息
 */
typedef void (^TuSDKTSAssetsManagerGroupsBlock)(NSArray<TuTSAssetsGroupInterface> *groups, NSError *error);
 ...
 *  @param error 错误对象
 */
typedef void (^TuSDKTSAssetsManagerSaveImageCompletion)(id<TuTSAssetInterface> asset, NSError* error);
 ...
 *  @param error 错误对象
 */
typedef void (^TuSDKTSAssetsManagerSaveImageAblumCompletion)(id<TuTSAssetsGroupInterface> group, id<TuTSAssetInterface> asset, NSError* error);
