...
@class TuSDKPFEditImageView;

/**
 *  图片编辑视图 (旋转，缩放)委托
 */
@protocol TuSDKPFEditImageViewDelegate <NSObject>
/**
 *  动画状态改变
 *
 ...
 */
- (void)onTuSDKPFEditImageView:(TuSDKPFEditImageView *)view inAniming:(BOOL)inAniming;
@end

/**
 *  图片编辑视图 (旋转，缩放)
 */
@interface TuSDKPFEditImageView : UIView<UIScrollViewDelegate>{
@protected
    // 旋转和裁剪 裁剪区域视图
 ...
 *  图片编辑视图 (旋转，缩放)委托
 */
@property (nonatomic, assign) id<TuSDKPFEditImageViewDelegate> delegate;
